...
do
    ...
    then
        ...
        object_key_path=`jq '.Body.Records[0].s3.object.key' <<< "$first_message"`
        object_key_path=`sed 's/\"//g' <<< $object_key_path`
        object_key=`sed 's|.*/||' <<< $object_key_path`

        aws s3 cp "${bucket_name}${object_key_path}" ~/sq.mp4

        ffmpeg -i ~/sq.mp4 -vf scale=320:240 ~/lq.mp4

        new_path="${bucket_name}${object_key}"
        aws s3 cp ~/lq.mp4 "${new_path}/lq.mp4"
        aws s3 cp ~/sq.mp4 "${new_path}/sq.mp4"
        aws s3 rm "${bucket_name}${object_key_path}"
